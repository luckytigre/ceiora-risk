...
- `backend/universe/runtime_rows.py` helper delegation to `runtime_authority` is landed
...
- a later fallback-scope review reopened three narrower repo-side follow-ups:
  ...
  - source-observation authority selection was later landed as its own narrow follow-on
  - broader runtime behavior work still needs a clean slice that does not bundle historical precedence or legacy-master candidate-anchor changes
...
- the remaining blockers are therefore no longer rollout-only
  - the re-cut broader runtime behavior slice
...
- the remaining work is the re-cut broader runtime behavior cleanup, the final compatibility retirement decision, the destructive rollout sequence, and the operational evidence required to close the plan fully
...
- [x] update [ARCHITECTURE_AND_OPERATING_MODEL.md](/Users/shaun/Library/CloudStorage/Dropbox/045%20-%20Vibing/ceiora-risk/docs/architecture/ARCHITECTURE_AND_OPERATING_MODEL.md) in the same working-tree slice that changed cUSE operating authority, source-sync authority, or shared-read authority
- [ ] update [CPAR_ARCHITECTURE_AND_OPERATING_MODEL.md](/Users/shaun/Library/CloudStorage/Dropbox/045%20-%20Vibing/ceiora-risk/docs/architecture/CPAR_ARCHITECTURE_AND_OPERATING_MODEL.md) in the same commit that changes cPAR package scope, runtime coverage semantics, or read contracts
- [x] update [cUSE4_engine_spec.md](/Users/shaun/Library/CloudStorage/Dropbox/045%20-%20Vibing/ceiora-risk/docs/reference/specs/cUSE4_engine_spec.md) in the same working-tree slice that changed cUSE stage logic, membership fields, or payload semantics
- [x] update [UNIVERSE_ADD_RUNBOOK.md](/Users/shaun/Library/CloudStorage/Dropbox/045%20-%20Vibing/ceiora-risk/docs/reference/protocols/UNIVERSE_ADD_RUNBOOK.md) before or with any change to registry seed format, operator workflow, or selector behavior
- [ ] update [TRBC_CLASSIFICATION_PIT_PROTOCOL.md](/Users/shaun/Library/CloudStorage/Dropbox/045%20-%20Vibing/ceiora-risk/docs/reference/protocols/TRBC_CLASSIFICATION_PIT_PROTOCOL.md) before or with any change to PIT classification ingest scope or readiness semantics
- [ ] update [NEON_CANONICAL_SCHEMA.sql](/Users/shaun/Library/CloudStorage/Dropbox/045%20-%20Vibing/ceiora-risk/docs/reference/migrations/neon/NEON_CANONICAL_SCHEMA.sql) and [NEON_CPAR_SCHEMA.sql](/Users/shaun/Library/CloudStorage/Dropbox/045%20-%20Vibing/ceiora-risk/docs/reference/migrations/neon/NEON_CPAR_SCHEMA.sql) in the same commit that introduces a schema dependency in code
...
- `backend/universe/source_observation.py`
  - changed authority selection so registry-backed rows remain primary while compat-only rows still materialize when the registry is populated for other names
- `backend/universe/security_master_sync.py`
...
- `backend/tests/test_universe_migration_scaffolding.py`
  - existing source-observation authority tests now cover the landed registry-primary plus compat-only anti-join behavior
...
- `3 passed` on:
  - `backend/tests/test_universe_migration_scaffolding.py -k source_observation_requires_populated_registry_before_switching_authority or source_observation_keeps_compat_only_rows_when_registry_is_populated_for_other_names or source_observation_can_materialize_registry_first_surfaces_without_security_master`
- `1 passed` on:
...
- re-cut and land the broader runtime behavior slice without bundling historical precedence or legacy-master candidate-anchor changes
