...
- Update the canonical merge input.
- Merge the new names into `security_master`.
...
- duplicate RIC check
- blank `ric` / blank `ticker` check
- expected delta in `classification_ok = 1` and `is_equity_eligible = 1`

Recommended spot checks:
- sample a few new names and verify `ticker`, `ric`, `exchange_name`, and classification fields
