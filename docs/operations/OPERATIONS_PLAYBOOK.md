...
- In that mode:
  ...
  - `backend/data/serving_outputs.py` remains the boundary module for serving payload reads, writes, and repair/manifest checks; higher layers should not import `backend/data/serving_output_read_authority.py`, `backend/data/serving_output_write_authority.py`, or `backend/data/serving_output_manifest.py` directly
...
- `serve-refresh`: quick serving refresh; no core recompute and no source ingest.
  ...
  - workspace scratch `data_db` / `cache_db` inputs do not force `core_reads` local on an ordinary serving-only lane; workspace paths alone should not override the backend-selection choice already in effect for that run
  - projection-only registry rows plus persisted projected-loadings fallback may still read explicit workspace/canonical SQLite paths inside the refresh context; that exception should stay narrow and should not silently retarget the rest of the serving lane
...
- In the default Neon-authoritative path, those rebuild lanes execute in this order:
  ...
  - mirror artifact persistence, runtime-health publication, and offline parity repair now route through `backend/services/neon_mirror_reporting.py`
