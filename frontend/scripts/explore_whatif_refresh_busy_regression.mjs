  ...
  try {
    ...
    await page.route("**/api/**", async (route) => {
      ...
      if (method === "POST" && pathName === "/api/refresh") {
        counters.refreshPost += 1;
        if (counters.refreshPost === 1) {
          return fulfillJson({
            status: "busy",
        ...
        return fulfillJson({
          ...
          refresh: {
            ...
            force_core: false,
            force_risk_recompute: false,
            requested_at: "2026-03-14T14:00:02Z",
            started_at: "2026-03-14T14:00:02Z",
            finished_at: null,
            current_stage: "serving_refresh",
            current_stage_message: "Publishing serving payloads",
            result: null,
      ...
      if (method === "GET" && pathName === "/api/refresh/status") {
        ...
        if (counters.refreshStatus === 1) {
          return fulfillJson({
            ...
            refresh: {
              ...
              requested_at: "2026-03-14T14:00:00Z",
              started_at: "2026-03-14T14:00:00Z",
              finished_at: "2026-03-14T14:00:01Z",
              current_stage: null,
              current_stage_message: null,
              result: { status: "ok", run_id: "run_old" },
              error: null,
            },
          });
        }
        return fulfillJson({
          ...
          refresh: {
            ...
            requested_at: "2026-03-14T14:00:02Z",
            started_at: "2026-03-14T14:00:02Z",
            finished_at: "2026-03-14T14:00:04Z",
            current_stage: null,
            current_stage_message: null,
            result: { status: "ok", run_id: "run_new" },
            error: null,
          },
        });
      }
      ...
      if (method === "GET" && pathName === "/api/operator/status") {
        counters.operatorStatus += 1;
        if (counters.refreshPost === 0) {
          return fulfillJson({
            status: "ok",
            ...
            refresh: { status: "idle", job_id: null, result: null, error: null },
            holdings_sync: { pending: false, pending_count: 0, dirty_revision: 0, last_refresh_started_dirty_revision: 0 },
          });
        }
        if (counters.refreshPost === 1) {
          return fulfillJson({
            status: "ok",
            generated_at: "2026-03-14T14:00:01Z",
            lanes: [],
            source_dates: {},
            risk_engine: {},
            core_due: { due: false, reason: "not_due" },
            refresh: {
              status: counters.refreshStatus === 0 ? "running" : "ok",
              job_id: "job_old",
              pipeline_run_id: "run_old",
              profile: "serve-refresh",
              requested_profile: "serve-refresh",
            ...
            holdings_sync: {
              ...
              pending_count: 1,
              dirty_revision: 2,
              last_refresh_started_dirty_revision: 1,
            },
          });
        }
        return fulfillJson({
          status: "ok",
          generated_at: "2026-03-14T14:00:04Z",
          lanes: [],
          source_dates: {},
          risk_engine: {},
          core_due: { due: false, reason: "not_due" },
          ...
          holdings_sync: {
            ...
            pending_count: 0,
            dirty_revision: 2,
            last_refresh_started_dirty_revision: 2,
          },
        });
      }
    ...
    await waitForCondition(
      () => counters.refreshPost === 2,
      15000,
      "Expected busy refresh helper to retry after the older run finished",
    );
    assert.equal(
      counters.portfolio,
      1,
      "Portfolio should not revalidate before the retry refresh finishes",
    );
    ...
    assert.equal(counters.refreshPost, 2, "Busy refresh path should post twice");
    assert.ok(counters.refreshStatus >= 2, "Busy refresh path should wait for both the older and retry refresh jobs");
    assert.equal(
      counters.portfolio,
      2,
      "Portfolio should revalidate once after the retry refresh reaches a terminal state",
    );

    console.log("Explore what-if busy refresh regression passed.");
  } finally {
    await browser.close();
  }
} catch (error) {
  console.error("Explore what-if busy refresh regression failed.");
  console.error(error);
  if (debugCounters) {
