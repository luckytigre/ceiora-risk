...
export default function SectionRegression({ data }: { data: HealthDiagnosticsData }) {
  ...
  const structuralColor = chartVar("--analytics-market", chartShortColor(0.9));
  const styleBlockColor = chartVar("--analytics-style", chartLongColor(0.9));
    ...
    datasets: [
      {
        ...
        borderColor: structuralColor,
        backgroundColor: (ctx) => {
          const chart = ctx.chart;
          if (!chart.chartArea) return alphaColor(structuralColor, 0.15);
          const grad = chart.ctx.createLinearGradient(0, chart.chartArea.top, 0, chart.chartArea.bottom);
          grad.addColorStop(0, alphaColor(structuralColor, 0.38));
          grad.addColorStop(0.55, alphaColor(structuralColor, 0.14));
          grad.addColorStop(1, alphaColor(structuralColor, 0.04));
          return grad;
        },
        borderWidth: 1,
        pointRadius: 0,
        pointHoverRadius: 3,
        pointHoverBackgroundColor: structuralColor,
      ...
      },
      {
        ...
        borderColor: styleBlockColor,
        backgroundColor: (ctx) => {
          const chart = ctx.chart;
          if (!chart.chartArea) return alphaColor(styleBlockColor, 0.12);
          const grad = chart.ctx.createLinearGradient(0, chart.chartArea.top, 0, chart.chartArea.bottom);
          grad.addColorStop(0, alphaColor(styleBlockColor, 0.32));
          grad.addColorStop(0.55, alphaColor(styleBlockColor, 0.12));
          grad.addColorStop(1, alphaColor(styleBlockColor, 0.03));
          return grad;
        },
        borderWidth: 1,
        pointRadius: 0,
        pointHoverRadius: 3,
        pointHoverBackgroundColor: styleBlockColor,
        pointHoverBorderColor: chartTextColor("primary"),
        pointHoverBorderWidth: 1.5,
        fill: "-1",
        stack: "r2_blocks",
      ...
      },
      {
        ...
        borderColor: chartTextColor("primary", 0.35),
        borderWidth: 1.2,
        pointRadius: 0,
        pointHoverRadius: 3,
        pointHoverBackgroundColor: chartTextColor("primary", 0.9),
        pointHoverBorderColor: chartTextColor("primary"),
        pointHoverBorderWidth: 1.5,
        fill: false,
    ...
    datasets: [
      {
        ...
        borderColor: chartShortColor(0.9),
        borderWidth: 1.5,
        pointRadius: 0,
        tension: 0.2,
      },
        ...
        {hasBlockRows && (() => {
          ...
          return (
            <div className="detail-history-stats">
              <span className="detail-history-stat" style={{ color: structuralColor }}>
                Struct {structuralR2.toFixed(1)}%
              </span>
              <span className="detail-history-stat" style={{ color: styleBlockColor }}>
                +Style {styleR2.toFixed(1)}%
              </span>
              <span className="detail-history-stat" style={{ color: chartTextColor("primary", 0.7) }}>
                = {fullR2.toFixed(1)}%
              </span>
            </div>
          );
