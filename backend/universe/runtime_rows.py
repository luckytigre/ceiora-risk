...
def _load_policy_rows(conn: sqlite3.Connection) -> dict[str, dict[str, Any]]:
    if not _table_exists(conn, SECURITY_POLICY_CURRENT_TABLE):
        return {}
    rows = conn.execute(
        f"""
        SELECT
...
def _load_taxonomy_rows(conn: sqlite3.Connection) -> dict[str, dict[str, Any]]:
    if not _table_exists(conn, SECURITY_TAXONOMY_CURRENT_TABLE):
        return {}
    rows = conn.execute(
        f"""
        SELECT
...
def _load_latest_source_observations(
    ...
    *,
    ...
    if not _table_exists(conn, SECURITY_SOURCE_OBSERVATION_DAILY_TABLE):
        return {}
    params: list[Any] = []
    where = ""
...
def load_security_runtime_rows(
    ...
    *,
    ...
    registry_table_exists = _table_exists(conn, SECURITY_REGISTRY_TABLE)
