...
- Universe definition is centralized in `security_master` (SID-keyed).
- Eligible investable set is controlled by:
  - `classification_ok = 1`
  - `is_equity_eligible = 1`
- As of this spec revision, the active eligible universe is approximately 3,000 names (current DB run: 3,019 SIDs).
- Distinct tickers can be lower than distinct SIDs due to share classes/listing aliases.
...
- Canonical persisted source-of-truth tables are:
  ...
  - `security_prices_eod`
- Legacy table names are compatibility views only and are not canonical write targets.
...
Persist ESTU audit artifacts per `(date, sid)`.
- `estu_flag` (0/1)
...
- `roa_pct` <- `TR.F.ReturnAvgTotAssetsPct`
...
- `ticker_ric_map`, `fundamental_snapshots`, `trbc_industry_history`, `prices_daily`, `fundamentals_history`, and `trbc_industry_country_history` are compatibility views only.
- No new writes should target compatibility views.
...
- Profitability: equal-weight composite:
  ...
  - 25% ROA
  - 25% CFO / Assets
  - 25% Sales Margin (operating margin proxy)
...
1. Universe and identity layer:
   - Maintain `security_master` as the single universe authority (SID, PermID, RIC, ticker, classification flags).
...
1. Complete monthly fundamentals PIT backfill through current date for full eligible universe.
2. Rebuild `universe_cross_section_snapshot` under `mode=current` to remove historical carryover rows from earlier runs.
3. Keep compatibility views read-only and continue migrating any residual legacy readers to canonical-first reads.
4. Continue scheduled coverage audits (fundamentals/classification/prices) and ESTU drop-reason monitoring.
